...
(function(){
  ...
  NetNS.prototype['delete'] = function(cb){
    ...
    if (this._exists()) {
      nameSuffix = this._long;
      last2Octets = this.ipAddress.replace(/^\d+\.\d+\./, '');
      return _execSeries(["ip netns del ns" + nameSuffix, "ip link del d" + nameSuffix, "iptables -t nat -D PREROUTING -d " + this.ipAddress + " -j DNAT --to 10." + last2Octets + ".1", "iptables -t nat -D POSTROUTING -s 10." + last2Octets + ".0/31 -j SNAT --to " + this.ipAddress], function(err){
        if (err) {
          return cb(err);
        } else {
          return cb(void 8);
