...
(function(){
  var child_process, fs, ip, async, _namespaces, _testUrl, _deleteAllRetries, _deleteAllDelay, _ports, _procFile;
  ...
  _ports = [80, 443];
  _procFile = '/proc/sys/net/ipv4/ip_forward';
  ...
  NetNS.prototype.create = function(cb){
    var this$ = this;
    return fs.readFile(_procFile, function(err, setting){
      if (err || !setting.toString().match(/^1/)) {
        console.error("Warning: " + _procFile + " must be set to 1 for NetNS to function properly.");
      }
      return this$._exists(function(err, exists){
        ...
        if (err) {
          ...
          if (this$.preRouting) {
            if (!preRoutingExists) {
              commands.push("iptables -t nat -A PREROUTING -d " + this$.ipAddress + "/32 -p tcp --match multiport --dports " + _ports.join(',') + " -j DNAT --to-destination 10." + last2Octets + ".1");
            }
          }
  ...
  NetNS.prototype['delete'] = function(cb){
    var this$ = this;
    return this._exists(function(err, exists){
      var nameSuffix, last2Octets, commands;
      if (err) {
        return cb(err);
      } else if (exists === true || exists === null) {
        nameSuffix = this$._long;
        last2Octets = this$.ipAddress.replace(/^\d+\.\d+\./, '');
        commands = ["ip netns del ns" + nameSuffix, "ip link del d" + nameSuffix];
        if (this$.preRouting) {
          commands.push("iptables -t nat -D PREROUTING -d " + this$.ipAddress + "/32 -p tcp --match multiport --dports " + _ports.join(',') + " -j DNAT --to-destination 10." + last2Octets + ".1");
        }
        commands.push("iptables -t nat -D POSTROUTING -s 10." + last2Octets + ".0/31 -j SNAT --to " + this$.ipAddress);
        return async.eachSeries(commands, function(cmd, cb){
          return child_process.exec(cmd, function(err, stdout, stderr){
            if (err) {
