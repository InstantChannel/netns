...
(function(){
  var child_process, fs, ip, _namespaces, _testUrl, _deleteAllRetries, _deleteAllDelay;
  ...
  NetNS.prototype['delete'] = function(cb){
    ...
    if (this._exists()) {
      nameSuffix = this._long;
      last2Octets = this.ipAddress.replace(/^\d+\.\d+\./, '');
      return _execSeries(["ip netns del ns" + nameSuffix, "ip link del d" + nameSuffix, "iptables -t nat -D PREROUTING -d " + this.ipAddress + " -j DNAT --to 10." + last2Octets + ".1", "iptables -t nat -D POSTROUTING -s 10." + last2Octets + ".0/31 -j SNAT --to " + this.ipAddress], true, function(err){
        if (err) {
          return cb(err);
        } else {
          return cb(void 8);
  ...
  function _execSeries(cmds, noErrors, cb){
    var execNext;
    execNext = function(){
  ...
  process.once('beforeExit', NetNS.deleteAll);
  process.once('SIGINT', function(){
    return NetNS.deleteAll(function(){
